...
#Creating an example table an getting Data back to python
#cursorSQL.execute(
"""
drop table if exists ClaasTab;
"""
#)

#cursorSQL.execute(
"""
CREATE TABLE ClaasTab
(
   DEPTCODE   INT(10),
   DeptName   CHAR(30),
   LOCATION   VARCHAR(33)
...
#cursorSQL.execute(
"""
INSERT INTO ClaasTab VALUES (10, 'FINANCE', 'EDINBURGH'),
...
for x in res:
	...
	newDoc = { "att1": x[1], "att2": [x[2], x[0]], "att3": x[1], "Date": d }
...
countPoduct = 0
for ecahProduct in allProducts:
	name = ecahProduct[1]
	price = ecahProduct[2]
	indications = ecahProduct[3]

	campains = []
	cursorSQL.execute(
	"""
	SELECT * FROM Campaign
	WHERE ID_Product="""+ str(ecahProduct[0])
	)
	...
	WHERE sql11501710.Product.ID_product="""+ str(ecahProduct[0])
	)
	...
	countPoduct += 1
print(f"{countPoduct} new Products inserted to DB")


#TODO Is this sql part only fetching data and responsible for the migration?

try: 
		
	#---------Employee-------

	def inserEmployee(name, gender, salery, teamLeader, hirDate):
	...
	for eachEmployee in allEmployees:
		...
		inserEmployee(name, gender, salery, teamLeader, hirDate)
		employeeCount += 1
	print(f"{employeeCount} new Employees inserted to DB")
	...
	def creatRegion(eachRegion):
		res = []
		res.append({"Name":eachRegion[0]})
		res.append({"General_Manager":creatGm(eachRegion[1])})
	...
	def insertRegion(name, countries, gm):
		...
		#for eachRegion in regions:
			#regionsInser.append(creatRegion(eachRegion))
	...
	#name = "England"
	#regions = [["London", ["Samir Arabi", "male", "5000", ["10", "5", "2007"], ["Qeen", "Pope"]]], ["Birmingthon", ["Samir Arabi", "male", "5000", ["10", "5", "2007"], ["Qeen", "Pope"]]]]

	#inserCountry(name, regions )
	...
	def findGM(regionID, cursorSQL):
		...
		return {"Name": str(GM[0][3])+" "+ str(GM[0][4]), "Gender": GM[0][5], "Salery": int(GM[0][6]), "Team_Leader": GM[0][7], "Hir_Date": creatDate(GM[0][8])}
	...
	for eachRegion in allRegions:
		...
		countRegion += 1
	print(f"{countRegion} new Regions inserted to DB")
	...
	for ecahClient in res:
		...
		countClient += 1
	print(f"{countClient} new Client inserted to DB")
	...
	for eachOrder in res:
		...
		countOrder += 1
	print(f"{countOrder} new Orders added to DB")
...
def Af1():
	...
	if count == 1:
		...
		if count2 == 1:
			for document in cursor:
				res = "The follwoing pro was inserted" + str(document)
	print(res)
	return res
...
def Af2():
	...
	if count == 1:
		...
		if count2 == 1:
			...
			if count3 == 1:
				for document in cursor:
					res = "The follwoing Client was inserted" + str(document)
		else:
			res= "THe compnay does not surply the country"
	print(res)
	return res
