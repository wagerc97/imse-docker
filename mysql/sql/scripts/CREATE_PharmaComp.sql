...
-- ---------------------------------------------
-- REVENUE VIEW (HELPER VIEW)    # IMPORTANT #
-- ---------------------------------------------
-- This is a helper view to access Advertises the revenue generated by each order
...
-- ORDERS VIEW   # ? #
-- ---------------------------------
-- The view gathers all the important info from ORDERS and joins the helper VIEW revenues to get access to the revenue
CREATE OR REPLACE VIEW ord_rev AS
    SELECT o.ID_Orders, p.Product_Name, c.Client_Name, o.Quantity, r.revenue, o.Order_Date
    FROM Orders o
    ...
    GROUP BY o.ID_Orders, r.revenue, o.Order_Date, p.Product_Name, 
            c.Client_Name, o.Quantity
  --  ORDER BY revenue DESC
;
-- SELECT * FROM ord_rev ORDER BY Order_Date DESC LIMIT 5;


-- -----------------------------------------------------
-- Most recent orders VIEW (HELPER VIEW for IMSE)
-- -----------------------------------------------------
-- This is a helper view to list the 5 most recent orders or less if there are less than 5 orders overall
CREATE OR REPLACE VIEW list_recent_orders AS
    SELECT  o.id_orders, p.product_name, o.Order_date
    FROM Orders o
        INNER JOIN Product p ON p.id_product = o.id_product
    GROUP BY o.id_orders, p.product_name, p.Order_date
